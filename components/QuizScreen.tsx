    ...
    <div className="w-full max-w-3xl mx-auto p-4 flex flex-col h-full justify-center">
      ...
      <div className="flex justify-between items-center mb-6 bg-white p-4 rounded-xl shadow-sm">
        <div className="flex flex-col">
          <span className="text-sm text-slate-500 font-semibold uppercase tracking-wider">Soru</span>
          <span className="text-2xl font-bold text-slate-800">{currentIndex + 1} <span className="text-slate-400 text-lg">/ {totalQuestions}</span></span>
        </div>
        <div className="flex flex-col items-end">
          <span className="text-sm text-slate-500 font-semibold uppercase tracking-wider">Puan</span>
          <span className={`text-2xl font-bold transition-colors duration-300 ${currentScore >= 0 ? 'text-indigo-600' : 'text-red-500'}`}>
            {currentScore}
          </span>
        </div>
      </div>
      ...
      {/* Progress Bar */}
      <div className="w-full bg-slate-200 rounded-full h-2.5 mb-10">
        <div 
      ...
      {/* Question Card */}
      <div className="bg-white rounded-2xl shadow-xl p-8 md:p-12 mb-8 relative overflow-hidden transition-all duration-300">
        {/* Status indicator bar on the left */}
        <div className={`absolute top-0 left-0 w-2 h-full transition-colors duration-300 ${
          feedbackState === 'idle' ? 'bg-indigo-500' : 
          feedbackState === 'correct' ? 'bg-green-500' : 'bg-red-500'
        }`}></div>
        
        <div className="mb-6 text-sm bg-indigo-50 text-indigo-700 inline-block px-3 py-1 rounded-full font-medium">
          İpucu: {question.hint}
        </div>

        <form onSubmit={handleSubmit} className="text-2xl md:text-3xl leading-relaxed text-slate-800 font-medium">
          ...
          <input
            ...
            className={`border-b-4 outline-none px-2 py-1 mx-1 w-40 text-center transition-colors placeholder:text-slate-300 rounded ${inputBorderClass}`}
            placeholder="___"
            autoComplete="off"
            autoCorrect="off"
            autoCapitalize="none"
